...
date: 2017-05-08 15:16:13
tags: [api]
---

## 接口说明

由ZhihuDailyPurify整理的[文档](https://github.com/izzyleung/ZhihuDailyPurify/wiki/%E7%9F%A5%E4%B9%8E%E6%97%A5%E6%8A%A5-API-%E5%88%86%E6%9E%90)

##  源码

[源码](https://github.com/shouDeveloper/zhihu-daily-api)

## 使用

### 启动服务器
```
git clone https://github.com/shouDeveloper/zhihu-daily-api.git
cd zhihu-daily-api
npm run start
```
### 客户端使用

只要把上述文档中的host更改为本地的host即可
例如 `http://news-at.zhihu.com/api/4/news/latest`则转为`http://localhost:9999/api/4/news/latest`
对于`图片`，也是吧其host改成本地host即可。

## API文档

### 1. 启动界面图像获取
* URL: `/api/7/prefetch-launch-images/1080*1920`  
...
### 2. 软件版本查询
* Android: `/api/4/version/android/2.3.0`
* iOS: `/api/4/version/ios/2.3.0`
...
### 4. 消息内容获取与离线下载
* URL: `/api/4/news/3892357`  
* 使用在 `最新消息` 中获得的 `id`，拼接在 `/api/4/news/` 后，得到对应消息 JSON 格式的内容
* 响应实例：
...
### 5. 过往消息
* URL: `/api/4/news/before/20131119`  
...
### 7. 新闻对应长评论查看
* URL: `/api/4/story/8997528/long-comments`
* 使用在 `最新消息` 中获得的 `id`，在 `/api/4/story/#{id}/long-comments` 中将 `id` 替换为对应的 `id`，得到长评论 JSON 格式的内容
* 响应实例：
...
### 8. 新闻对应短评论查看
* URL: `/api/4/story/4232852/short-comments`
* 使用在 `最新消息` 中获得的 `id`，在 `/api/4/story/#{id}/short-comments` 中将 `id` 替换为对应的 `id`，得到短评论 JSON 格式的内容
* 响应实例：
...
### 10. 主题日报内容查看
* URL: `/api/4/theme/11`
* 使用在 `主题日报列表查看` 中获得需要查看的主题日报的 `id`，拼接在 `/api/4/theme/` 后，得到对应主题日报 JSON 格式的内容
* 响应实例：
...
* Android: `/api/3/promotion/android`  
* iOS: `/api/3/promotion/ios`
...
* 往前：`/api/3/section/1/before/1398780001`
    * 在 URL 最后加上一个时间戳，时间戳详见 JSON 数据末端的 `timestamp` 属性



### 15. 查看新闻的推荐者
* URL: `/api/4/story/#{id}/recommenders`
* 将新闻id填入到#{id}的位置， 如 `/api/4/story/7101963/recommenders`
* 响应实例
...
### 16. 获取某个专栏之前的新闻
* URL: `/api/4/section/#{section id}/before/#{timestamp}`
* 将专栏id填入到 #{section id}, 将时间戳填入到#{timestamp}，如 `/api/4/section/34/before/1465772400`
...
### 17. 查看 Editor 的主页
* iOS: `/api/4/editor/#{id}/profile-page/ios`
* Android: `/api/4/editor/#{id}/profile-page/android`
* 如 /api/4/editor/79/profile-page/ios